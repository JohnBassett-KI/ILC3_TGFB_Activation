library('Seurat')
load("data/gca_adult_large_intestine/gca_adult_LargInt_Seurat_QC.robj")
...
#Remove all cells with total counts over 20k
gca_finalQC <- subset(gca_finalQC, subset = nCount_originalexp <= 20000)
VlnPlot(gca_finalQC,features=c('total_counts'), pt.size = 0, group.by = 'Integrated_05', adjust = 2) + NoLegend()
...
gca_adult_largeIntQC_downsampled <- subset(gcA_samp, cells = retain)

#write.csv(Cells(gca_adult_largeIntQC_downsampled), "data/gca_downsampled_cell_names.csv",
#          row.names = F, )

gca_adult_largeIntQC_downsampled <- RenameAssays(object = gca_adult_largeIntQC_downsampled, originalexp = "RNA")
#gca_adult_largeIntQC_downsampled <- NormalizeData(gca_adult_largeIntQC_downsampled, normalization.method ="LogNormalize")
save(gca_adult_largeIntQC_downsampled, file='data/gca_adult_large_intestine/gca_adult_largeIntQC_downsampled.robj')
...
#write counts file to a format readable by cellphone db
